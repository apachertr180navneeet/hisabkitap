...
<head>
  ...
  <title>{{ $pso->code }} Collection & Settlement Breakdown - {{ date('d/m/Y', strtotime($businessDate)) }} | HisabKitap ERP</title>
...
<body>
  ...
  <div class="no-print bg-dark text-white py-2 px-3 sticky-top shadow-sm">
    <div class="container d-flex flex-wrap justify-content-between align-items-center gap-2" style="max-width: 1050px;">
      <div class="d-flex align-items-center gap-2">
        ...
        <span class="fw-bold">{{ $pso->code }} Detail Breakdown Report</span>
        <span class="badge bg-secondary font-mono">{{ date('d M Y', strtotime($businessDate)) }}</span>
      </div>
      <div class="d-flex gap-2">
        <a href="{{ route('admin.summary.show', $pso->id) }}" class="btn btn-outline-light btn-sm">
          <i class="bi bi-arrow-left me-1"></i> Back
        </a>
        <a href="{{ route('admin.summary.export_single_excel', $pso->id) }}" class="btn btn-success btn-sm">
  ...
  <div class="print-container">
    <div class="d-flex justify-content-between align-items-start border-bottom pb-3 mb-3">
      <div class="d-flex align-items-center gap-2">
        ...
        <div>
          ...
          <span class="text-muted small">Single PSO Counter Bill Register & Category Breakdown</span>
        </div>
      </div>
      <div class="text-end">
    ...
    <div class="row g-2 mb-3 p-3 bg-light rounded border">
      <div class="col-3">
        <span class="text-muted small d-block">PSO Code & Prefix:</span>
        <strong class="font-mono text-primary fs-6">{{ $pso->code }} ({{ $pso->prefix }})</strong>
      </div>
      <div class="col-3">
        <span class="text-muted small d-block">Series Range:</span>
        <strong class="font-mono text-dark">{{ sprintf('%02d', $pso->start_no) }} to {{ sprintf('%02d', $pso->end_no) }}</strong>
      </div>
      <div class="col-3">
        <span class="text-muted small d-block">Operator / Driver:</span>
        <strong class="text-dark">{{ $pso->operator_name }} @if($pso->driver_name) | {{ $pso->driver_name }} @endif</strong>
      </div>
      <div class="col-3 text-end">
        <span class="text-muted small d-block">Vehicle / Gadi No:</span>
        <strong class="font-mono text-dark">{{ $pso->gadi_number ?: 'Counter Desk' }}</strong>
      </div>
    </div>
    ...
    <div class="row g-2 mb-3">
      <div class="col">
        <div class="border rounded p-2 text-center bg-white">
          <small class="text-muted d-block fw-semibold">Gross Sales</small>
          <span class="fs-6 fw-bold font-mono text-dark">₹{{ number_format($stats['gross'], 2) }}</span>
        </div>
      </div>
      <div class="col">
        <div class="border rounded p-2 text-center bg-white">
          <small class="text-muted d-block fw-semibold">Cash Portion</small>
          <span class="fs-6 fw-bold font-mono text-success">₹{{ number_format($stats['cash'], 2) }}</span>
        </div>
      </div>
      <div class="col">
        <div class="border rounded p-2 text-center bg-white">
          <small class="text-muted d-block fw-semibold">Paytm / Digital</small>
          <span class="fs-6 fw-bold font-mono text-info">₹{{ number_format($stats['paytm'], 2) }}</span>
        </div>
      </div>
      <div class="col">
        <div class="border rounded p-2 text-center bg-white">
          <small class="text-muted d-block fw-semibold">Cheques</small>
          <span class="fs-6 fw-bold font-mono text-primary">₹{{ number_format($stats['check'], 2) }}</span>
        </div>
      </div>
      <div class="col">
        <div class="border rounded p-2 text-center bg-white">
          <small class="text-muted d-block fw-semibold">Net Collection</small>
          <span class="fs-6 fw-bold font-mono text-warning">₹{{ number_format($stats['net'], 2) }}</span>
        </div>
      </div>
    </div>
    ...
    <table class="table table-bordered table-sm align-middle text-center mb-4">
      ...
      <tbody>
        @forelse($bills as $b)
          ...
          @endphp
          <tr>
            <td class="font-mono fw-bold text-primary">{{ $b->bill_no }}</td>
        ...
        @empty
          <tr>
            <td colspan="9" class="py-3 text-muted">No bill records found for {{ $pso->code }}.</td>
          </tr>
        @endforelse
      </tbody>
      <tfoot class="table-dark">
        <tr>
          <th colspan="5" class="text-start ps-3">TOTAL ({{ count($bills) }} BILLS):</th>
          <th class="text-end font-mono">₹{{ number_format($stats['gross'], 2) }}</th>
          <th class="text-end font-mono text-danger">-₹{{ number_format($stats['cd'] + $stats['refund'], 2) }}</th>
          <th class="text-end font-mono text-warning">₹{{ number_format($stats['net'], 2) }}</th>
          <th></th>
        </tr>
      </tfoot>
    </table>
    ...
    <div class="row g-4 mt-4">
      <div class="col-4"><div class="signature-box"><div class="fw-bold">{{ $pso->operator_name ?: 'Operator' }}</div><small class="text-muted">PSO Operator Signature</small></div></div>
      <div class="col-4"><div class="signature-box"><div class="fw-bold">Cashier / Receiver</div><small class="text-muted">Counted & Received By</small></div></div>
      <div class="col-4"><div class="signature-box"><div class="fw-bold">Manager / Accounts</div><small class="text-muted">Audited & Verified</small></div></div>
    </div>
  </div>
